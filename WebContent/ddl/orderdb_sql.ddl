CREATE DATABASE orders;
go;

USE orders;
go;

DROP TABLE review;
DROP TABLE shipment;
DROP TABLE productinventory;
DROP TABLE warehouse;
DROP TABLE orderproduct;
DROP TABLE incart;
DROP TABLE product;
DROP TABLE category;
DROP TABLE author;
DROP TABLE ordersummary;
DROP TABLE paymentmethod;
DROP TABLE customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE author (
    authorId             INT IDENTITY,
    authorName           VARCHAR(50),
    PRIMARY KEY (authorId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(100),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    authorId            INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId),
    FOREIGN KEY (authorId) REFERENCES author(authorId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Biography'); --1
INSERT INTO category(categoryName) VALUES ('Children''s'); --2
INSERT INTO category(categoryName) VALUES ('Fantasy'); --3
INSERT INTO category(categoryName) VALUES ('Conspiracy'); --4
INSERT INTO category(categoryName) VALUES ('Mystery'); --5
INSERT INTO category(categoryName) VALUES ('Non-Fiction'); --6
INSERT INTO category(categoryName) VALUES ('Poetry'); --7
INSERT INTO category(categoryName) VALUES ('Romance'); --8

INSERT INTO author(authorName) VALUES ('Mike Jeffress'); --1 done
INSERT INTO author(authorName) VALUES ('Cornelius Implorium'); --2 done
INSERT INTO author(authorName) VALUES ('Biggles Tall'); --3 done
INSERT INTO author(authorName) VALUES ('Genevieve Serene'); --4 done
INSERT INTO author(authorName) VALUES ('Ace Monteque'); --5 done
INSERT INTO author(authorName) VALUES ('Anonymous'); --6 done
INSERT INTO author(authorName) VALUES ('Greg Vartin'); --7 done
INSERT INTO author(authorName) VALUES ('Jim Scuttleson'); --8 done
INSERT INTO author(authorName) VALUES ('Lucas Dark'); --9 done
INSERT INTO author(authorName) VALUES ('Sir Woof'); --10 done
INSERT INTO author(authorName) VALUES ('Susie Saltwater'); --11 done
INSERT INTO author(authorName) VALUES ('Bella Jacobs'); --12 done
INSERT INTO author(authorName) VALUES ('B.E. Kind'); --13 done

INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('The Man Behind The Falls',1,1,'To behold the splendor of the Sequoia Falls is to gaze upon the greatest beauty that nature has to offer',25.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Famous Spellcasters Throughout History',1,2,'The history of spellcasting is long and extensive.',25.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Chasing The Quadruple Rainbow',2,3,'A harrowing account of the search for the most dreaded weather phenomenon.',20.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('How I Dumped Your Father',2,4,'Kids, this story involves corgis, a slap bet, and a convincing Bella Goth imposter. Buckle up!',22.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('The Adventures of Pink Ruffles',3,5,'Shamed by the pirate community solely for his name, Pink Ruffles sailed the seas with his trusty crew. ',1.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Avoiding The Watchful Eye',4,6,'101 tips on avoiding unwanted snoops and spies.',21.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Why are We Here?',4,6,'The answers to all of life''s greatest mysteries. The true reason you are stuck in this canyon.',30.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('A Game of Groans',3,7,'An epic fantasy series of feuding kingdoms, fascinating characters, and the complex power struggles to control those kingdoms.',25.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Crisis Barn',5,8,'When humble family man Ira Strauss left the big city to start a farm, he never expected to find a dead French diplomat face-down in the hay.',3.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Lucas Dark and The Increasingly Unlikely Series of Coincidences',5,9,'This gripping mystery tells the tale of Lucas Dark, an exceedingly attractive author with nothing left to lose but his mind.',1.50);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('1001 Tricks You Can Teach Your Familiar',6,1,'All the tricks you could possibly teach your familiar. Too bad they are always so stubborn they never actually learn.',21.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Gentleman Corgi: A Memoir',6,10,'Sir Woof is the World''s Most Gentlemanly Corgi, a title bestowed by the Distinguished Canine Society.',24.50);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('The 50 Shades On My Windows',7,2,'One is light gray. Another is slate. And there''s a charcoal one.',23.25);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('The Impromptu Impromptu',7,3,'Surprise your listeners with the Impromptu Impromptu! ',15.50);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Love In the Time of Sandwiches',8,4,'Tiny Tino and Gina are in love, but Gina bends to pressure and marries another.',1.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Packs and Prejudice',8,5,'Lizo Barknet is perfectly happy running his pack as Alpha leader - his rules, no questions asked.',35.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Seduction by the Sea',8,11,'The perfect blend of passion and paragraphs..',29.50);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('My Pizza-cat-o Polka',2,7,'This piece was written as an homage to Buchholz''s cat, Fiona.',9.20);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Dusk Glow',3,12,'This is the classic that started it all! Supernatural romance at its best! ',25.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Dusk Glow 2: Moonrise',3,12,'How does Darla handle it when her secret-vampire boyfriend suddenly disappears from her life? She doesn’t! ',20.00); --20
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('The Cherry Lip Balm Caper',5,8,'Red lips sink ships, at least in Hergstrom they do. ',60.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Beyond Big and Bad: Rethinking Harmful Werewolf Stereotypes',6,13,'Beyond Big & Bad delves deep into today''s rampant mischaracterization of the Werewolf occult.',20.99);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Cooking With Motes',6,9,'	Add a little extra "kick" to your boring, weekday recipes!',25.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Elegy for a Fallen Piano',6,11,'A challenging piece that requires clean pedaling in order to make this song sound even halfway decent.',39.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Yay, Pork Barrels',7,10,'	A collection of delirious poems written by one man after successfully eating a hogshead barrel full of bacon.',18.40);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Seven Course Death',8,12,'A rainy night. An old mansion. A dinner party for the toast of Hergstrom''s upper crust.',25.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Wands of Desire',8,13,'Julie has always been cold when it comes to showing her feelings, but can a dashing old spellcaster melt her heart?',14.00);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Peter Barker''s Homecoming Hijinks',2,1,'Things take a mischievous turn as Peter Barker returns home.',21.05);
INSERT product(productName, categoryId, authorId, productDesc, productPrice) VALUES ('Fortune Telling for Goofballs',4,6,'	7000 Tips and tricks to truly believe you are an effective fortune teller.',20.00);

INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (1, 1, 5, 25);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (2, 1, 10, 25);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (3, 1, 3, 20);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (4, 1, 2, 22);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (5, 1, 6, 1);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (6, 1, 3, 21);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (7, 1, 1, 30);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (8, 1, 0, 25);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (9, 1, 2, 3);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (10, 1, 3, 1.50);

INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');

-- Order 1 can be shipped as have enough inventory
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2019-10-15 10:25:55', 28.50)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 2, 1.00)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 1.50);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-16 18:00:00', 5.00)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 5, 1.00);

-- Order 3 cannot be shipped as do not have enough inventory for item 7
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2019-10-15 3:30:22', 132)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 21)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 7, 3, 30);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-17 05:45:11', 346.85)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 3, 4, 20)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 8, 3, 25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 3, 23.25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 28, 2, 21.05)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 29, 4, 20);


--New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'img/1_1.jpg' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/1_2.jpg' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/2.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/2_2.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/3.jpg' WHERE ProductId = 5;
UPDATE Product SET productImageURL = 'img/4.jpg' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/4_2.jpg' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/3.jpg' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/2_2.jpg' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/2_2.jpg' WHERE ProductId = 10;
UPDATE Product SET productImageURL = 'img/6.jpg' WHERE ProductId = 11;
UPDATE Product SET productImageURL = 'img/6_2.jpg' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'img/7.jpg' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'img/7.jpg' WHERE ProductId = 14;
UPDATE Product SET productImageURL = 'img/8.jpg' WHERE ProductId = 15;
UPDATE Product SET productImageURL = 'img/8_2.jpg' WHERE ProductId = 16;
UPDATE Product SET productImageURL = 'img/8_3.jpg' WHERE ProductId = 17;
UPDATE Product SET productImageURL = 'img/2.jpg' WHERE ProductId = 18;
UPDATE Product SET productImageURL = 'img/3.jpg' WHERE ProductId = 19;
UPDATE Product SET productImageURL = 'img/3_2.jpg' WHERE ProductId = 20;
UPDATE Product SET productImageURL = 'img/2_2.jpg' WHERE ProductId = 21;
UPDATE Product SET productImageURL = 'img/1_1.jpg' WHERE ProductId = 22;
UPDATE Product SET productImageURL = 'img/6_3.jpg' WHERE ProductId = 23;
UPDATE Product SET productImageURL = 'img/1_1.jpg' WHERE ProductId = 24;
UPDATE Product SET productImageURL = 'img/7.jpg' WHERE ProductId = 25;
UPDATE Product SET productImageURL = 'img/8_3.jpg' WHERE ProductId = 26;
UPDATE Product SET productImageURL = 'img/8_4.jpg' WHERE ProductId = 27;
UPDATE Product SET productImageURL = 'img/2_3.jpg' WHERE ProductId = 28;
UPDATE Product SET productImageURL = 'img/3_3.jpg' WHERE ProductId = 29;

-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;