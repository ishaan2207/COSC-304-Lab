...
{	
	...
	if (mycustId == 69420) { //probably change this so it does the below code if logged in as admin (this is basically the lab 8 solution untouched)
		...
		out.println("</table>");
	} else { //do if logged in
		String userName = (String) session.getAttribute("authenticatedUser");
		out.print("<h1>Order List For "+ userName + "</h1>");
		String sql = "SELECT orderId, orderSummary.customerId, totalAmount, firstName+' '+lastName, orderDate FROM ordersummary JOIN customer ON ordersummary.customerId = customer.customerId WHERE orderSummary.customerId = ?";
		PreparedStatement pstmt1 = con.prepareStatement(sql);
		pstmt1.setInt(1,mycustId);
